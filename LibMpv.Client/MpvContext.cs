...
public unsafe partial class MpvContext : IDisposable
{
    public MpvContext()
    {
        Console.WriteLine("MpvContext 1");
        handle = LibMpv.MpvCreate();
        Console.WriteLine("MpvContext 2");
        ...
        Console.WriteLine("MpvContext 3");

        var result = LibMpv.MpvInitialize(handle);
        Console.WriteLine("MpvContext 4" + result);
        ...
        var version = LibMpv.MpvClientApiVersion();
        Console.WriteLine("MpvContext 5:" + version);

        eventLoop = new(handle, HandleMpvEvent);
        eventLoop.Start();
        Console.WriteLine("MpvContext 6");
    }
    ...
    public void AddOverlay(uint id, uint x, uint y, byte[] imageData, string format, uint width, uint height,
        uint stride)
    {
    ...
    private static object _lockWin = new();
