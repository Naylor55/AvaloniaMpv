...
public abstract partial class BaseMpvContextViewModel : MpvContext
{
    ...
    {
        Console.WriteLine("BaseMpvContextViewModel1");
        //InitPropertyObserver();
        Console.WriteLine("BaseMpvContextViewModel2");
    ...
    {
        Console.WriteLine("Context_FileLoaded");
    ...
    {
        Console.WriteLine("Context_EndFile");
    ...
    private void AddQueue(string key, object action)
    {
        // actionQueue.Add(new(key, action));
    }

    protected virtual void InitPropertyObserver()
    {
        // actionQueue = new ActionQueue<(string, object)>((item) =>
        // {
        //     if (item.Item2 is Action<bool> ab)
        //     {
        //         this.ObserveProperty(item.Item1, ab);
        //     }
        //     else if (item.Item2 is Action a)
        //     {
        //         this.ObserveProperty(item.Item1, a);
        //     }
        // });
        return;
        ...
        this.ObserveProperty("cache-buffering-state",
            () => RaisePropertyChangedInUIThread(nameof(CacheBufferingState)));
